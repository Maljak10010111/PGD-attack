...
class AutoAttack():
    def __init__(self, model, rejection_class_index=10, norm='Linf', eps=.3, seed=None, verbose=True,
        ...
        if not self.is_tf_model:
            from modified_autopgd_base import APGDAttack
            ...
            from modified_autopgd_base import APGDAttack_targeted
            ...
            for attack in attacks_to_run:
                ...
                for batch_idx in range(n_batches):
                    ...
                    if attack == 'apgd-ce':
                        ...
                        adv_curr = self.fab.perturb(x, y)
                    # MODIFIED
                    elif attack == 'apgd-ce-rejection':
                        # apgd on dlr loss
                        self.apgd.loss = 'ce-rejection'
                        self.apgd.seed = self.get_seed()
                        adv_curr = self.apgd.perturb(x, y)  # cheap=True
                    elif attack == 'apgd-dlr-rejection':
                        # apgd on dlr loss
                        self.apgd.loss = 'dlr-rejection'
                        self.apgd.seed = self.get_seed()
                        adv_curr = self.apgd.perturb(x, y)  # cheap=True
                    else:
