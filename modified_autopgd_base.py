...
class APGDAttack():
    ...
    def attack_single_run(self, x, y, x_init=None):
        ...
        # MODIFIED
        def rejection_ce(logits, y):
            criterion_indiv = nn.CrossEntropyLoss(reduction='none')
            return criterion_indiv(logits, y) + criterion_indiv(logits, torch.full_like(y, self.rejection_class_index))

        def rejection_dlr(logits, y):
        ...
        if not self.is_tf_model:
            ...
            # MODIFIED
            elif self.loss == 'ce-rejection':
                criterion_indiv = rejection_ce
            elif self.loss == 'dlr-rejection':
                criterion_indiv = rejection_dlr
            else:
    ...
    def perturb(self, x, y=None, best_loss=False, x_init=None):
        ...
        # MODIFIED
        assert self.loss in ['ce', 'dlr', 'ce-rejection', 'dlr-rejection']  # 'ce-targeted-cfts'
